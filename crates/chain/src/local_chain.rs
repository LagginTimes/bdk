...
pub struct ChangeSet<B>(BTreeMap<u32, Option<B>>);
...
#[derive(Debug, Clone)]
pub struct CheckPoint<B>(Arc<CPInner<B>>);
    ...
    B: Copy + Clone + core::fmt::Debug,
{
    fn eq(&self, other: &Self) -> bool {
        let self_cps = self.iter().map(|cp| cp.block_id().as_ref().clone());
        let other_cps = other.iter().map(|cp| cp.block_id().as_ref().clone());
    ...
    B: Copy + Clone + core::fmt::Debug,
{
    ...
    pub fn from_blocks(blocks: BTreeMap<u32, B>) -> Result<Self, MissingGenesisError> {
        ...
        for block in &blocks {
            match tip {
                Some(curr) => tip = Some(curr.push(block.1.clone()).expect("BTreeMap is ordered")),
                None => tip = Some(CheckPoint::new(block.1.clone())),
            }
        }
        ...
        if let Some(start_height) = changeset.keys().next().cloned() {
            ...
            for cp in self.iter() {
                if cp.height() >= start_height {
                    extension.insert(cp.height(), cp.inner().clone());
                } else {
                    base = Some(cp);
                    break;
                }
            ...
            let new_tip = match base {
                Some(base) => base
                    .extend(extension.values().map(|&b| b.clone()))
...
pub struct LocalChain<B: AsRef<BlockId>>
where
    B: Copy + Clone + core::fmt::Debug,
{
    tip: CheckPoint<B>,
    ...
    B: Copy + Clone + core::fmt::Debug,
{
    ...
    fn get_chain_tip(&self) -> Result<BlockId, Self::Error> {
        Ok(self.tip.block_id().as_ref().clone())
    }
}
...
impl LocalChain<BlockId> {
    ...
    pub fn from_changeset(changeset: ChangeSet<BlockId>) -> Result<Self, MissingGenesisError> {
        ...
        let (mut chain, _) = Self::from_genesis_hash(genesis_hash.hash);
    ...
    fn _check_changeset_is_applied(&self, changeset: &ChangeSet<B>) -> bool {
        ...
        for (height, exp_block) in changeset.0.iter().rev() {
            match curr_cp.get(*height) {
                Some(query_cp) => {
                    if query_cp.height() != *height || Some(query_cp.inner().clone()) != *exp_block
                    {
                        return false;
                    }
                    curr_cp = query_cp;
...
where
    B: AsRef<BlockId> + core::fmt::Debug + Copy + Clone,
{
    let mut changeset = ChangeSet::<B>::default();
    ...
    loop {
        ...
        match (curr_orig.as_ref(), curr_update.as_ref()) {
            ...
            (o, Some(u)) if Some(u.height()) > o.map(|o| o.height()) => {
                changeset.0.insert(u.height(), Some(u.inner().clone()));
