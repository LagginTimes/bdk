...
/// Width of the elapsed time stdout.
...
fn main() -> anyhow::Result<()> {
    ...
    println!(
        "[{:>ELAPSED_WIDTH$}s] loaded initial changeset from db",
    ...
    println!(
        "[{:>ELAPSED_WIDTH$}s] loaded indexed tx graph from changeset",
        start.elapsed().as_secs_f32()
    );

    let chain = Mutex::new(LocalChain::from_changeset(init_changeset.0));
    println!(
        "[{:>ELAPSED_WIDTH$}s] loaded local chain from changeset",
        start.elapsed().as_secs_f32()
    );
    ...
    match rpc_cmd {
        RpcCommands::Sync { rpc_args } => {
            ...
            while let Some((height, block)) = emitter.next_block()? {
                ...
                if last_db_commit.elapsed() >= DB_COMMIT_DELAY {
                    ...
                    db.commit()?;
                    println!(
                        "[{:>ELAPSED_WIDTH$}s] commited to db (took {}s)",
                        start.elapsed().as_secs_f32(),
                ...
                if last_print.elapsed() >= STDOUT_PRINT_DELAY {
                    ...
                    if let Some(synced_to) = chain.tip() {
                        ...
                        println!(
                            "[{:>ELAPSED_WIDTH$}s] synced to {} @ {} | total: {} sats",
                            start.elapsed().as_secs_f32(),
                            synced_to.hash(),
                            synced_to.height(),
                            balance.total()
        ...
        RpcCommands::Live { rpc_args } => {
            ...
            println!(
                "[{:>ELAPSED_WIDTH$}s] starting emitter thread...",
                start.elapsed().as_secs_f32()
            );
